    ...
    def _train(self, sess):
        ...
        for i_epoch in range(self.params['max_epoches']):
            ...
            for img_frame_vecs, img_frame_n, ques_vecs, ques_n, ques_word, ans_vecs, ans_n, ans_word, type_vec, batch_size in self.train_batcher.generate():
                ...
                batch_data[self.model.target] = ans_word
                batch_data[self.model.frame_vecs] = img_frame_vecs
                batch_data[self.model.frame_len] = img_frame_n
                batch_data[self.model.ques_vecs] = ques_vecs
                batch_data[self.model.ques_len] = ques_n
                batch_data[self.model.answer_vecs] = ans_vecs
                ...
                for ind, row in enumerate(mask_matrix):
                    row[:ans_n[ind]] = 1
                batch_data[self.model.target_mask] = mask_matrix

                batch_t1 = time.time()
                _, train_loss, train_ans, learning_rate = sess.run([train_proc, self.model.train_loss, self.model.answer_word_train, learning_rates], feed_dict=batch_data)
